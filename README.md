...
│  News │ Sources                                         │
│  PDF Export button │ Excel Export button                │
...
| **PDF / Excel Export** | One-click export of the full analysis report — PDF (cover page, report, financial tables, valuation, peer comparison, debate transcript) and Excel workbook (5 sheets: Summary, Financial Statements, Valuation, Peer Comparison, Debate Transcript) |
...
| **Web UI** | Vue 3 + SSE streaming, Single/Compare modes, 7-tab panel, floating AI chat drawer, PDF/Excel export buttons |
...
Export         fpdf2 (PDF generation) + openpyxl (Excel workbook)
...
│   │   ├── exporter.py              # PDF (fpdf2) + Excel (openpyxl) report generation
...
After analysis completes, **PDF** and **Excel** buttons appear in the panel header alongside "Ask AI":

```
NVDA  [STRONG BUY]  [STRONG]    [PDF]  [Excel]  [Ask AI]
```

**PDF Report** — multi-page document generated by `fpdf2`:

| Section | Content |
|---------|---------|
| Cover Page | Ticker, investment rating badge, live price summary, generated timestamp |
| Investment Report | Full final report (markdown stripped) |
| Financial Statements | Income statement, balance sheet, cash flow tables |
| Valuation Analysis | DCF table + relative multiples table + methodology note |
| Peer Comparison | Peer metrics table + strengths/weaknesses |
| Debate Transcript | Full Analyst ↔ Critic exchange by round |

**Excel Workbook** — multi-sheet `.xlsx` generated by `openpyxl`:

| Sheet | Content |
|-------|---------|
| Summary | Key metrics (rating, price, 52W range, market cap) + full report text |
| Financial Statements | Income statement, balance sheet, cash flow (separate sub-sections) |
| Valuation | DCF model + relative multiples |
| Peer Comparison | Peer metrics table + competitive positioning |
| Debate Transcript | Round-by-round debate with speaker coloring |

Files are downloaded directly in the browser as `{TICKER}_analysis.pdf` / `{TICKER}_analysis.xlsx`.
...
| `POST` | `/api/export/pdf` | Binary: generate and return a PDF investment report |
| `POST` | `/api/export/excel` | Binary: generate and return an Excel workbook (`.xlsx`) |
...
| `fpdf2` | PDF report generation (pure Python) |
